    ...
    const gerente = await User.findOne({puesto: 'Gerente de Operaciones'})
        const smtpTransport = nodemailer.createTransport({
          ...
          mailOptions = {
            ...
            subject: reason,
            text: 'Ha recibido este correo ya que se ha utilizado la forma en el sitio web.\n\n' +
              'Proviene de: '+ name +'\n\n' +
              'Con su forma de contacto indicada: '+ contact +'\n\n' +
              'Siendo su correo:\n\n' + text +'\n\n' 
          };
...
router.post('/sendmail', catchAsync(async(req, res) => {
...
router.get('/show/:id/edit', isLoggedIn, catchAsync(async(req, res) => {
    try{
        const customer = await customerSchema.findById(req.params.id)
...
router.put('/show/:id/edit', isLoggedIn, catchAsync(async(req, res) => {
    const customer = await customerSchema.findByIdAndUpdate(req.params.id, req.body)
    await customer.save()
    res.redirect(`/clientes/${customer._id}`)
}))

router.delete('/show/:id', isLoggedIn, catchAsync(async(req, res)=>{
    const {id} = req.params;
